...
// import Stackedbar from "@/app/components/Stackedbar";
  ...
  useEffect(() => {
    const getData = async () => {
      const course = await stuData.filter(
        (item) => item.department == "Department of Business Administration"
      );
      // console.log(course);
      getBaStudent(course);
    };
    ...
    const getData2 = async () => {
      const course = await emData.filter(
        (item) => item.department == "Business Administration"
      );
      // console.log(course);
            ...
            {baEmployee.map((item, index) => (
              <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700" key={index}>
                <th className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                  <Link href={`/Course/BA/employee/${item._id}`}>
        ...
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            ...
            {baStudent.map((item, index) => (
              <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700" key={index}>
                <th className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                  <Link href={`/Course/BA/student/${item._id}`}>
